...
def AM(t):
    ...
    m.setObjective(lambda_1[0]*delta_r + quicksum(s[i]*o[i] for i in range(random_size))/float(random_size)-(1-epsilon)*beta[0], GRB.MINIMIZE)
    m.update()
    ...
    m.addConstrs(s[j] >=beta[0] for j in range(random_size))
...
#     while iteration<=maxiter and Difference>1e-5:
        ...
        iteration +=1
        g_val=np.dot(a,x_s)
#         support=np.count_nonzero(g_val>10+1e-5)
...
#     print('current checking alsox is : ', checking)
...
#         t_2=t
        checking_1= AM(t)
#         print('current checking  am is : ', checking_1)
        if checking_1<=0:
            t_1=t
